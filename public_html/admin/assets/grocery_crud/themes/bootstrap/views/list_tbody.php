...
<?php foreach($list as $num_row => $row){ ?>
    <tr>
        ...
        <td <?php if ($unset_delete) { ?> style="border-left: none;"<?php } ?>
            <?php if ($buttons_counter === 0) {?>class="hidden"<?php }?>>
                <div class="only-desktops"  style="white-space: nowrap">
                    <?php if(!$unset_edit){?>
                        <a class="btn btn-default" href="<?php echo $row->edit_url?>"><i class="fas fa-pencil-alt"></i> <?php echo $this->l('list_edit'); ?></a>
                    <?php } ?>
                        ...
                        <ul class="dropdown-menu">
                            ...
                            <?php }
                             if (!$unset_delete) { ?>
                                <li>
                                    <a data-target="<?php echo $row->delete_url?>" href="javascript:void(0)" title="<?php echo $this->l('list_delete')?>" class="delete-row">
                                        <i class="fa fa-trash text-danger"></i> <span class="text-danger"><?php echo $this->l('list_delete')?></span>
                                    </a>
                                </li>
                            <?php } ?>
                        </ul>
        ...
        <?php
            if(!empty($row->action_urls)){
                foreach($row->action_urls as $action_unique_id => $action_url){
                    $action = $actions[$action_unique_id];
                    ?>
                        <a href="<?php echo $action_url; ?>" class="<?php echo $action->css_class; ?> crud-action" title="<?php echo $action->label?>" data-toggle="tooltip" data-placement="top" title="<?php echo $action->label?>">
                        </a>
                <?php }
            }
        ?>
        ...
        <?php if (!$unset_clone) { ?>
                <a href="<?php echo $row->clone_url?>">
                    <i class="fas fa-copy m-r-5"></i>
                </a>
        <?php } ?>
        <?php if (!$unset_delete) { ?>
                <a data-target="<?php echo $row->delete_url?>" href="javascript:void(0)" title="<?php echo $this->l('list_delete')?>" class="delete-row">
                    <i class="fas fa-trash text-danger m-r-5"></i>
                </a>
        <?php } ?>
        </td>
    </tr>
